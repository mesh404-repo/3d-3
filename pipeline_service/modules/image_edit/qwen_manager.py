...
class QwenManager:
    ...
    async def _load_pipeline(self) -> None:
        ...
        # quantize(self.pipe.transformer, weights=qint8)
        ...
        try:
            from para_attn.first_block_cache import apply_cache_on_pipe
            apply_cache_on_pipe(self.pipe)
            logger.info("First Block Cache applied to Qwen pipeline")
        except ImportError:
            logger.warning("para-attn not installed, skipping First Block Cache")
        except Exception as e:
            logger.warning(f"Failed to apply First Block Cache: {e}")
