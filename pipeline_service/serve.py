...
async def generate_from_base64(request: GenerateRequest) -> GenerateResponse:
    ...
    """
    try:
        result_3_views, result_1_views = await pipeline.generate_gs(request)
        # Use 3-view result for base64 endpoint
        result = result_3_views
...
async def generate_spz(prompt_image_file: UploadFile = File(...), seed: int = Form(-1)) -> StreamingResponse:
    ...
    try:
        ...
        # Compress the ply file
        if ply_bytes:
            ply_compressed_bytes = pyspz.compress(ply_bytes, workers=1)  # returns bytes
