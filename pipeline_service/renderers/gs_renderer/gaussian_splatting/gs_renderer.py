...
from renderers.gs_renderer.gaussian_splatting.gs_camera import OrbitCamera
...
class GaussianSplattingRenderer:
    ...
    def __init__(self) -> None:
        ...
        self._bg_color = torch.tensor([0, 0, 0], dtype=torch.float32).to(self._device)
    ...
    def setup_cameras(
        ...
        Args:
            theta_angles: Azimuth angles (yaw) in degrees
            phi_angles: Elevation angles (pitch) in degrees
        """
        ...
        # theta_angles = azimuth (yaw), phi_angles = elevation (pitch)
        ...
        # theta = azimuth (yaw), phi = elevation (pitch)
        for azimuth, elevation, j in zip(thetas, phis, range(views_number), strict=False):
            self._camera.compute_transform_orbit(elevation, azimuth, cam_rad, is_degree=True)
    ...
    def render(self, gs_data: GaussianSplattingData, bg_color: torch.Tensor | None = None) -> list[torch.Tensor]:
        ...
        rendered_rgba = torch.cat([rendered_images, rendered_alphas], dim=-1)

        # converting tensors to image-like tensors, keep all of them in device memory
        output_rendered_images = [(img * 255).to(torch.uint8) for img in rendered_rgba]
